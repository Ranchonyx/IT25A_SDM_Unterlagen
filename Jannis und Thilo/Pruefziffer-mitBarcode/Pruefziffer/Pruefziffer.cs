...
namespace Pruefziffer
{
    public partial class Pruefziffer : Form
    {
        ...
        private int PruefzifferBerechnen(int[] ziffer, int[] faktor)
        {
            ...
            {
                summe[i] = ziffer[i] * faktor[i]; // Summe aus den Ziffern und Faktoren bilden
                int tempSumme = summe[i];
                while (tempSumme > 0)
                {
                    quersumme[i] += tempSumme % 10; // Quersumme der Summen bilden 
                    tempSumme /= 10;
                }
            }
        ...
        {
            if (mtb_nummer_new.Text.Length < 16) return;

            string nummer = mtb_nummer_new.Text;
            nummer = nummer.Replace("-", "");
            int[] ziffern = nummer.Select(c => int.Parse(c.ToString())).ToArray();
            string num = "";
            foreach (int z in ziffern) num += z;
            Debug.WriteLine(num);
            Barcode barcode = new Barcode();
            Color fColor = Color.Black;
            Color bColor = Color.Transparent;
            Image img = barcode.Encode(TYPE.CODE128, num, fColor, bColor, (int)(pb.Width * 0.8), (int)(pb.Height * 0.8));
            pb.Image = img;
        }
